...
Farmed from pydantic-ai's test suite and adapted to be standalone
...
from .harness_utils import raise_if_exception

T = TypeVar('T')


def sync_anext(iterator: Iterator[T]) -> T:
    """Return the next item from a synchronous iterator, or raise StopAsyncIteration."""
    try:
...
class MockAsyncStream(Generic[T]):
    ...
    async def __anext__(self) -> T:
        ...
        next_item = sync_anext(self._iter)
        raise_if_exception(next_item)
        return next_item
