...
const cases = [
  ...
  { task: "compare an indexed inspiration repository through Codebase Memory", expect: ["codebase-memory"], keywords: ["inspiration"], max: 1 },
  { task: "use JetBrains IDE search, refactoring, diagnostics, build, and run tools", expect: ["jetbrains-ide-workflow"], keywords: ["jetbrains"], max: 1 },
  ...
  { task: "one-session feature: code is ground truth, one-shot with examples", expect: ["codebase-driven-development"], keywords: ["ground truth"], max: 1 },
  { task: "turn a proven reusable code primitive into a skill for use a skill for reuse", expect: ["code-foundations"], keywords: ["proven"], max: 1 },
  { task: "port an agentic-harness primitive for live steering, result pairing, and compaction cuts from an indexed repo", expect: ["oh-my-pi-foundation"], keywords: ["steering"], max: 1 },
  { task: "switch from a planning model to an implementation model only after the first durable write", expect: ["oh-my-pi-prewalk-foundation"], keywords: ["handoff"], max: 1 },
  { task: "persist and rehydrate parent-scoped durable agent workers", expect: ["oh-my-pi-vibe-foundation"], keywords: ["durable"], max: 1 },
  { task: "build multi-agent task fanout with worktree isolation and cancellation-safe permits", expect: ["oh-my-pi-task-orchestration-foundation"], keywords: ["multi-agent"], max: 1 },
  { task: "run a metric-driven experiment loop on an isolated git branch", expect: ["oh-my-pi-autoresearch-foundation"], keywords: ["experiment"], max: 1 },
  { task: "route automated review findings to a live agent without unwanted auto-resume", expect: ["oh-my-pi-advisor-foundation"], keywords: ["advisor"], max: 1 },
