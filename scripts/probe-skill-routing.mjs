...
const cases = [
  ...
  { task: "re-squeeze the indexed Aider repository into source-grounded capsules", expect: ["foundations-workflow"], keywords: ["foundation"], max: 1 },
  { task: "port an agentic-harness primitive for live steering, repo, and compaction cuts from an indexed repo", expect: ["oh-my-pi-foundation"], keywords: ["steering"], max: 1 },
  { task: "switch to an implementation model only after the first durable write, then fan out workers", expect: ["oh-my-pi-foundation"], keywords: ["handoff", "durable"], max: 1 },
