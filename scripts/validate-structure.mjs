...
const REQUIRED_SCRIPTS = ["validate:structure", "validate:config", "validate:packs", "validate:research", "validate:skills", "validate:prompts", "validate:mcp", "validate:sources", "scan:secrets", "sync:sources", "typecheck", "test", "smoke:install", "check"];
...
export function main() {
  ...
  for (const doc of ["architecture.md", "operators.md", "sources.md", "research-routing.md", "verification.md"]) {
    if (!existsSync(join(root, "docs", doc))) errors.push("docs/" + doc + " missing");
  }
  if (errors.length > 0) return { ok: false, message: "structure: FAIL\n  - " + errors.join("\n  - ") };
  return { ok: true, message: "structure: OK — package, README (" + REQUIRED_HEADINGS.length + " headings), docs present" };
}
