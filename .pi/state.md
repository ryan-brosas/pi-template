...
No issue tracker or active bead is configured. Do not invent issue IDs. This file uses roadmap phases and concrete next actions until a tracker is explicitly adopted.

## Current Repository Condition

The checked-out branch is `main`, tracking `origin/main`. The working tree contains extensive edits that predate initialization:
...
| --- | --- | --- | --- |
| 2026-08-09 | Position the repository as a clonable pi.dev template inspired by `inspo/opencode-template` | Explicit user direction | Product vision, roadmap, and future comparison work |
| 2026-08-09 | Target developers | Explicit user choice | Documentation and defaults optimize for developer onboarding |
| 2026-08-09 | Prioritize stability | Explicit user choice | Safety, consistency, and drift detection outrank feature breadth |
| 2026-08-09 | Keep the active product Pi-native and Ultra Fabric-focused | README and current tree | Do not reintroduce OpenCode runtime layers without a separate decision |
| 2026-08-09 | Keep the template install-free | README states no build, dependencies, runtime harness, or package manifest | New tooling must not silently add a consumer install step |
...
There is no source-code execution graph, data layer, UI, deployment target, or CI pipeline in the current tree.
...
| Question | Why it matters | Blocking | Roadmap phase |
| --- | --- | --- | --- |
