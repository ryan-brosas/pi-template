...
Pick one primary route per question, escalate only on a named evidence gap, and stop once evidence is sufficient. This keeps research cheap and context small.
...
- **Parallel only for independent angles:** never fire the same question at several tools at once.
...
Fan-out across every tool; unbounded result counts; fetching every search hit; same question to two tools "to be safe"; treating a summary as a primary source; expanding a source before summarizing what it already gave.
