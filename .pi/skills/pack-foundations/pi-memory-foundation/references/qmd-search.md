...
**Probe:** `test/unit.test.ts` — `clampSearchLimit` describe (:2154): `defaults when undefined or NaN` (:2155), `clamps to the valid range and floors fractions` (:2160); `memory_search tool` describe (:1263): `defaults mode to keyword and limit to 5` (:1299); `getQmdSearchTimeoutMs` describe (:1254): `accepts positive integer milliseconds and defaults invalid values` (:1255). Coverage caveat: `test/` is excluded from the index by design, so probes are source-grounded from the on-disk test files.
