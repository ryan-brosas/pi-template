...
5. **Depth check** — a leaf at standard carries: provenance header per reference, 5W1H dissolved (not labeled), lessons + probes per section, and an explicit unmined-subsystems ledger in SKILL.md for future passes. Quantitative floors (minimums, never a cap): ≥10 files in `references/`, ≥700 lines per reference file; SKILL.md stays lean. Meet floors through stage-3 study (split subsystems finer, mine more probes), never padding.
   *Gate:* floors verified (`wc -l references/*`); compare against localterm side by side before proceeding.
...
- Floors, not caps: ≥10 reference files at ≥700 lines each is the **minimum** per repo; exceed freely when the repo warrants it. SKILL.md has no floor — the surface stays lean.
...
- `references/skill-anatomy.md` — target structure: lean surface anatomy, references split, depth floors (≥10 files × ≥700 lines each), validator constraints, provenance requirements.
