...
pack-foundations/<repo>-foundation/
  SKILL.md
  references/<question>.md   # one disting porting question each
```
...
2. Solves. 3. Reuse map (path/symbol + invariant + probe). 4. Full view (memory graph identity + caveats). 5. References. 6. Adopt/adapt/omit. 7. Unmined. 8. Result contract.
Remove any sentence that doesn't improve routing or reuse.
...
## What belongs where
Leaf: trigger, retrieval symbol+invariant, graph identity/caveat, porting summary, probe name. Capsule: exact flow + data shape, scoped coverage, mechanics, and minimal code shape when it prevents a wrong port. Many full implementations stay in the source repo.

## Mechanical checks
The validator checks anchors, provenance, probe signal, capsule structure (when mooted), scaffold leakage, and padding. Utility is by pressure test + `node scripts/check.mjs`.