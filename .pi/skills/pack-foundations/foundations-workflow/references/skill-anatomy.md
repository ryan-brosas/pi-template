...
## One canonical pair
Copy the foundation leaf and capsule exactly from `references/foundation-templates.md`. Portable mirrors live at `.pi/templates/foundation-skill.md` and `.pi/templates/foundation-capsule.md` (template-only library assets, not slash-command render targets). There is no second leaf or reference layout.
