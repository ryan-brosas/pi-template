...
```swift
// [x] Good: New code prepared for Swift 6
...
// [x] Rethink the problem with Swift Concurrency
Task {
...
Task { [weak self] in
    for await _ in NotificationCenter.default.notifications(named: .someNotification) {
        await self?.handleNotification()  // [x] Compile-time safe
    }
}
```
...
token = NotificationCenter.default.addObserver(
    ...
    self?.handleDidBecomeActive()  // [x] No warning, guaranteed main actor
}
```
...
token = NotificationCenter.default.addObserver(
    ...
    handleBuilds(message.recentBuilds)  // [x] Direct access, no casting
}
```
