...
```typescript
// [x] GOOD: Track processed messages
...
- [x] Design for idempotency (at-least-once delivery)
- [x] Use `json` content type for visibility
- [x] Log failures with context
- [x] Configure DLQ for permanent failures
- [x] Use `waitUntil()` for non-blocking sends
- [x] Batch sends when possible
